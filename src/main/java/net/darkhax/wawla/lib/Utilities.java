...
public class Utilities {
	...
	/**
	 * A simple boolean which will be true if the mod is launched in a developer
	 * environment. This field is set when the mod is launched, see the preInit
	 * of the Wawla mod for more info.
	 */
	public static boolean isDevMode = false;

	/**
	 * A simple method to make an ItemStack safe to work with in regards of nbt.
	 * If the stack does not have a tag compound one will be added. If it
	 * already has one then nothing will happen. This is useful when working
	 * with ItemStacks that may not have tag compounds yet.
	 * 
	 * @param stack
	 *            An instance of an ItemStack to be prepared for nbt work.
	 * @return ItemStack The same instance of ItemStack provided, however with a
	 *         tag compound added if one did not already exist.
	 */
	...
	/**
	 * This method will take a string and break it down into multiple lines
	 * based on a provided line length. The separate strings are then added to
	 * the list provided. This method is useful for adding a long description to
	 * an item tool tip and having it wrap. This method is similar to wrap in
	 * Apache WordUtils however it uses a List making it easier to use when
	 * working with Minecraft.
	 * 
	 * @param string
	 *            The string being split into multiple lines. It's recommended
	 *            to use StatCollector.translateToLocal() for this so multiple
	 *            languages will be supported.
	 * @param lnLength
	 *            The ideal size for each line of text.
	 * @param wrapLongWords
	 *            If true the ideal size will be exact, potentially splitting
	 *            words on the end of each line.
	 * @param list
	 *            A list to add each line of text to. An good example of such
	 *            list would be the list of tooltips on an item.
	 * @return List The same List instance provided however the string provided
	 *         will be wrapped to the ideal line length and then added.
	 */
	 ...
	 * 
	 * @param value
	 *            The double being round.
	 * @param places
	 *            The amount of places to round the double to.
	 ...
	 * @param enumClass
	 *            The enum class.
	 * @return String[] An array of strings that represent the names of the
	 *         elements in an enum.
	 */
	...
	/**
	 * Retrieves an inventory of items from an NBTTagCompound, useful for
	 * working with furnaces.
	 * 
	 * @param tag
	 *            The NBTTagCompound that contains an "Items" compound.
	 * @param invSize
	 ...
	 * 
	 * @param entity
	 *            The tile entity you wish to compare.
	 * @param teClass
	 *            The class of another tile entity.
	...
	/**
	 * Gets the progression percentage. Uses the current stage and the total
	 * maximum stage to make a percentage.
	 * 
	 * @param curStage
	 *            The stage you are current at.
	 * @param maxStage
	 *            The maximum possible stage for the event.
	 ...
	 * @param string
	 *            The string to use.
	 * @return string The input string, with the first character being upper
	 *         cased.
	 */
	 ...
	 * @return Class A class that maches the provided name. Can be null, if no
	 *         class is found.
	 */
	 ...
	 * 
	 * @param player
	 *            The player to check the sleepability of.
	...
	public static boolean canPlayerSleep(EntityPlayer player) {
		...
		return (!player.isPlayerSleeping() && player.isEntityAlive() && player.worldObj.getWorldTime() > 12541
	 ...
	 * 
	 * @param tag
	 *            An NBTTagCompound to search through.
	 * @param tags
	 *            An array containing the various steps to get to the desired
	 *            deep NBTTagCompound.
	...
	/**
	 * An accessible field which can be used to provide the client-side value of
	 * the current block damage. Initialized through the client proxy during
	 * preInit.
	 */
	public static Field currentBlockDamage;

	/**
	 * A client sided method used to retrieve the progression of the block
	 * currently being mined by the player. This method is client side only, and
	 * refers to only the one instance of the player. Do not try to use this
	 * method to get data for multiple players, or for server sided things.
	 * 
	 * @return float A float value representing how much time is left for the
	 *         block being broken to break. 0 = no damage has been done. 1 = the
	 *         block is broken.
	 */
