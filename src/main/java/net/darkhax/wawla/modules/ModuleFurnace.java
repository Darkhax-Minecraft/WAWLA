...
public class ModuleFurnace extends Module {
    ...
    public void onWailaBlockDescription(ItemStack stack, List<String> tooltip, IWailaDataAccessor access) {
        ...
        if (access.getTileEntity() != null && access.getTileEntity() instanceof TileEntityFurnace) {
            ...
            if (burnTime > 0)
                tooltip.add(StatCollector.translateToLocal("tooltip.burnTime") + ": " + burnTime + " " + StatCollector.translateToLocal("tooltip.seconds"));
            ...
            if (access.getPlayer().isSneaking()) {
                ...
                if (furnaceStacks[0] != null) 
                    tooltip.add(StatCollector.translateToLocal("tooltip.input") + ": " + furnaceStacks[0].getDisplayName() + " X " + furnaceStacks[0].stackSize);
                
                if (furnaceStacks[1] != null)
                    tooltip.add(StatCollector.translateToLocal("tooltip.fuel") + ": " + furnaceStacks[1].getDisplayName() + " X " + furnaceStacks[1].stackSize);
                
                if (furnaceStacks[2] != null)
                    tooltip.add(StatCollector.translateToLocal("tooltip.output") + ": " + furnaceStacks[2].getDisplayName() + " X " + furnaceStacks[2].stackSize);
            }
        }
    }
