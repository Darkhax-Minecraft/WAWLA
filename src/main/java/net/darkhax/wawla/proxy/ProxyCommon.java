package net.darkhax.wawla.proxy;

public class ProxyCommon {
    
    /**
     * The server side alternative to .registerSidedEvents(), this method should only be called
     * from the server side. The purpose of this method is to register events that are unique
     * to the server.
     */
    public void registerSidedEvents () {
    
    }
    
    /**
     * This method is used to register modules on the server.
     */
    public void registerSidedModules () {
    
    }
    
    /**
     * This method is used to do stuff postInit on the server.
     */
    public void sidedPostInit () {
    
    }
}
