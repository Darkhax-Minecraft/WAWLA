...
public class Utilities {
    ...
    /**
     * A simple method to make an ItemStack safe to work with in regards of nbt. If the stack does not have a tag compound one will be added. If it already has one then nothing will happen. This is useful when working with ItemStacks that may not have tag compounds yet.
     * 
     * @param stack: An instance of an ItemStack to be prepared for nbt work.
     * @return ItemStack: The same instance of ItemStack provided, however with a tag compound added if one did not already exist.
     */
     ...
     * 
     * @param string: The string being split into multiple lines. It's recommended to use StatCollector.translateToLocal() for this so multiple languages will be supported.
     ...
     * @param list: A list to add each line of text to. An good example of such list would be the list of tooltips on an item.
     * @return List: The same List instance provided however the string provided will be wrapped to the ideal line length and then added.
     */
     ...
     * @param stored: This boolean is used to differentiate between stored enchantments and actual enchantments. Enchantment Books do not keep their enchantments under the same compound as other enchanted items. Set to true if the ItemStack being checked has stored enchantments rather than active ones.
    ...
    /**
     * This method will create a chat component that includes both a string and a clickable link to the location of a file. This method should be sued when the player saves a file. Works similarly to the screenshot.
     * 
     * @param translationKey: The key to be used for translating the first part of the message. Example: command.print.sucess
     * @param file: The link to the file being linked. This is used to generate the file name and provide a link to that file.
    ...
    /**
     * This method writes an entity to a new NBTTagCompound. This is useful for getting information about an entity that has been set by vanilla. It is not very optimal however at this time it is the best way that I could find.
     * 
