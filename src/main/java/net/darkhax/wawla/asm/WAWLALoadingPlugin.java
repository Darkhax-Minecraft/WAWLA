...
public class WAWLALoadingPlugin implements IFMLLoadingPlugin 
{
	...
	private static final String CATEGORY_CLASS_TRANSFORMATIONS_COMMENT = 
			"This category contains configurable properties for performing certain class transformations. "
    ...
    {
    	Configuration config = new Configuration(new File((File) data.get("mcLocation"), "config/wawla.cfg"));
    	config.addCustomCategoryComment(CATEGORY_CLASS_TRANSFORMATIONS, CATEGORY_CLASS_TRANSFORMATIONS_COMMENT);
    	config.load();
    	WAWLAConfiguration.TRANSLATION_CLASS_TRANSFORM = config.get(CATEGORY_CLASS_TRANSFORMATIONS, "Perform Translation Hook Injection?", true);
    	config.save();
    }
